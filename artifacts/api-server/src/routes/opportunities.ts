...
export function createOpportunitiesRouter(
  ...
  router.get("/opportunities", requireAuth, async (req, res): Promise<void> => {
    ...
    const rows = await db
      ...
      .orderBy(
        asc(
          sql`CASE WHEN ${opportunitiesTable.deadline} IS NULL THEN 1 ELSE 0 END`,
        ),
        asc(opportunitiesTable.deadline),
      );
  ...
  router.patch(
    ...
    async (req, res): Promise<void> => {
      ...
      if (parsed.data.keyActionSteps !== undefined)
        updates.keyActionSteps = parsed.data.keyActionSteps;
      ...
      const completedTaskCount =
        (
          db
            ...
            .where(
              and(
                eq(tasksTable.opportunityId, id),
